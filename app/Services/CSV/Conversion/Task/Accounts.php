...
final class Accounts extends AbstractTask
{
    ...
    private function processTransaction(array $transaction): array
    {
        ...
        if ($this->hasAllCurrencies($transaction)) {
            ...
            if ('withdrawal' === $transaction['type']) {
                ...
                if ($transaction['currency_code'] !== $source['currency_code'] && $transaction['foreign_currency_code'] === $source['currency_code']) {
            ...
            if ('deposit' === $transaction['type']) {
                ...
                // so if we can switch them around we will.
                if (
                    $transaction['currency_code'] !== $destination['currency_code']
    ...
    {
        Log::debug('setTransactionAccount', $account);
        ...
        $transaction[sprintf('%s_number', $direction)] = null;
        if (array_key_exists('number', $account)) {
            $transaction[sprintf('%s_number', $direction)] = $account['number'];
        }
        if (array_key_exists('account_number', $account) && null === $transaction[sprintf('%s_number', $direction)]) {
